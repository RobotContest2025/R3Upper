...
RobotMode robot_state = ROBOT_IDEL;
...
void RemoteTask(void *param)
{
    ...
    Action_t action = {.param = &reset_flag, .action_cb = ResetAction, .type = ACTION_TYPE_UNINTERRUPTABLE}; // 发送复位动作
    ...
    while (1)
    {
       ...
       if (remoteRecv.Key.Left_Switch_Down_or_Right) // 左边按键控制运动模式
        ...
        if (remoteRecv.Key.Left_Key_Up == 1 && last_remoteRecv.Key.Left_Key_Up == 0) // 不论车辆处于哪种模式，按下此按键均将机器人恢复到复位模式
        {
            ...
            action.param = &reset_flag;
            xQueueSend(action_queue, &action, pdMS_TO_TICKS(10));
            robot_state = ROBOT_MOVE;
        }
        else if (remoteRecv.Key.Left_Key_Left == 1 && last_remoteRecv.Key.Left_Key_Left == 0) // 准备运球，运球动作按键
        {
            if (robot_state != ROBOT_DRIBBLE && robot_state != ROBOT_JUMP && robot_state != ROBOT_LAUNCH && dribble_ready_flag == 0 && reset_flag) // 机器人处于允许进入运球模式的状态时，才允许进入准备运球模式，执行准备运球动作
            {
                throw_ready_flag = 0;
                ready_jump_flag = 0;
                ...
                action.type = ACTION_TYPE_UNINTERRUPTABLE;
                action.param = &dribble_ready_flag;
                xQueueSend(action_queue, &action, pdMS_TO_TICKS(10));
                robot_state = ROBOT_READY_DRIBBLE;
            }
            else if (robot_state == ROBOT_READY_DRIBBLE && dribble_ready_flag)
            {
                dribble_ready_flag = 0;
                dribble_flag = 0;
                action.action_cb = DribbleAction2;
                action.type = ACTION_TYPE_INTERRUPTABLE;
                action.param = &dribble_flag;
                xQueueSend(action_queue, &action, pdMS_TO_TICKS(10));
                robot_state = ROBOT_DRIBBLE;
            }
        }
        else if (remoteRecv.Key.Left_Key_Down == 1 && last_remoteRecv.Key.Left_Key_Down == 0) // 准备投球，投球动作按键
        {
            if (robot_state != ROBOT_DRIBBLE && robot_state != ROBOT_JUMP && robot_state != ROBOT_LAUNCH && throw_ready_flag == 0 && reset_flag) // 允许进入投球模式安全检查
            {
                ready_jump_flag = 0;
                dribble_ready_flag = 0;
                ...
                action.type = ACTION_TYPE_UNINTERRUPTABLE;
                action.param = &throw_ready_flag;
                xQueueSend(action_queue, &action, pdMS_TO_TICKS(10));
                robot_state = ROBOT_READY_LAUNCH;
            }
            else if (robot_state == ROBOT_READY_LAUNCH && throw_ready_flag)
            {
                throw_ready_flag = 0;
                throw_flag = 0;
                action.action_cb = BackThrowAction;
                action.type = ACTION_TYPE_UNINTERRUPTABLE;
                action.param = &throw_flag;
                xQueueSend(action_queue, &action, pdMS_TO_TICKS(10));
                robot_state = ROBOT_LAUNCH;
            }
        }
        else if (remoteRecv.Key.Left_Key_Right == 1 && last_remoteRecv.Key.Left_Key_Right == 0) // 准备扣篮，扣篮动作按键
        {
            if (robot_state != ROBOT_DRIBBLE && robot_state != ROBOT_JUMP && robot_state != ROBOT_LAUNCH && ready_jump_flag == 0 && reset_flag) // 允许进入扣篮模式安全检查
            {
                throw_ready_flag = 0;
                dribble_ready_flag = 0;
                ...
                action.type = ACTION_TYPE_UNINTERRUPTABLE;
                action.param = &ready_jump_flag;
                xQueueSend(action_queue, &action, pdMS_TO_TICKS(10));
                robot_state = ROBOT_READY_JUMP;
            }
            else if (robot_state == ROBOT_READY_JUMP && ready_jump_flag)
            {
                ready_jump_flag = 0;
                jump_flag = 0;
                action.action_cb = JumpAction;
                action.type = ACTION_TYPE_UNINTERRUPTABLE;
                action.param = &jump_flag;
                xQueueSend(action_queue, &action, pdMS_TO_TICKS(10));
                robot_state = ROBOT_JUMP;
            }
        }
        else if (remoteRecv.Key.Right_Key_Up == 1 && last_remoteRecv.Key.Right_Key_Up == 0) // 防守动作按键
        {
            if (robot_state != ROBOT_DRIBBLE && robot_state != ROBOT_JUMP && robot_state != ROBOT_LAUNCH && reset_flag) // 当前状态允许进入防守模式，不会自动退出，执行其它任意动作终止此动作
            {
                defend_flag = 0;
                action.type = ACTION_TYPE_INTERRUPTABLE;
                action.action_cb = DefendAction;
                action.param = &defend_flag;
                xQueueSend(action_queue, &action, pdMS_TO_TICKS(10));
                robot_state = ROBOT_DEFEND;
            }
        }
        else if (remoteRecv.Key.Right_Key_Left == 1 && last_remoteRecv.Key.Right_Key_Left == 0)
        {
            if (robot_state != ROBOT_DRIBBLE && robot_state != ROBOT_JUMP && robot_state != ROBOT_LAUNCH && reset_flag) // 当前动作允许进入捡球模式，不会自动退出，执行其它任意动作终止此动作
            {
                pickup_flag = 0;
                action.type = ACTION_TYPE_INTERRUPTABLE;
                action.action_cb = PickUpBallAction;
                action.param = &pickup_flag;
                xQueueSend(action_queue, &action, pdMS_TO_TICKS(10));
                robot_state = ROBOT_PICKUP;
            }
        }
        ...
        else if (remoteRecv.Key.Right_Key_Right == 1 && last_remoteRecv.Key.Right_Key_Right == 0) // 待分配（可以给传球动作用）
        {
        }

        // 机器人状态更新
        if (dribble_flag && robot_state == ROBOT_DRIBBLE) // 完成运球后，将机器人切换为移动模式
        {
            dribble_flag = 0;
            robot_state = ROBOT_MOVE;
        }
        else if (throw_flag && robot_state == ROBOT_LAUNCH)
        {
            throw_flag = 0;
            robot_state = ROBOT_MOVE;
        }
        else if (jump_flag && robot_state == ROBOT_JUMP)
        {
            jump_flag = 0;
            robot_state = ROBOT_MOVE;
        }
