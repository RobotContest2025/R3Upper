...
//	throw_motor_vel_mode = 2;
	throw_motor_target_rad = throw_motor_reset_rad;
...
void ReadyDribbleAction2(void *param)
{
	...
	if (throw_motor.state.rad < ANGLE2RAD(13))
	{
		...
		if (lv53_cur_dis > 0.66f) // 调整运球起始大臂高度
		{
			jump_motor_target_vel = -2.0f;
			while (lv53_cur_dis > 0.66f)
				vTaskDelay(3);
		}
		else if (lv53_cur_dis < 0.66f)
		{
			jump_motor_target_vel = 2.0f;
			while (lv53_cur_dis < 0.66f)
				vTaskDelay(3);
	...
	throw_motor_vel_mode = 2; // 位置模式
	throw_motor_target_rad = ANGLE2RAD(90.0f - dribble_angle);

	if (lv53_cur_dis > dribble2_height - 0.05) // 调整运球起始大臂高度
	{
		jump_motor_target_vel = -2.0f;
		while (lv53_cur_dis > dribble2_height - 0.05)
			vTaskDelay(3);
		jump_motor_target_vel = 1.0f;
		while (lv53_cur_dis < dribble2_height)
			vTaskDelay(3);
	}
	else if (lv53_cur_dis < dribble2_height - 0.05)
	{
		jump_motor_target_vel = 2.0f;
		while (lv53_cur_dis < dribble2_height - 0.05)
			vTaskDelay(3);
...
void PickUpBallAction(void *param)
{
	...
	while (lv53_cur_dis < pickup_start_height)
		vTaskDelay(5);
...
float TargetSlope(float target, float current, float max_increase, float max_reduction)
{
	if (target - current > max_increase)		// 如果目标值大于当前值加上最大改变量
		return current + max_increase;			// 返回当前值加上最大改变量
	else if (target - current < -max_reduction) // 如果目标值小于当前值减去最大改变量
		return current - max_reduction;			// 返回当前值减去最大改变量
	else
