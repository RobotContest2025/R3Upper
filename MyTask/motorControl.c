...
void MotorControl(void *param)
{
	...
	RobStrideSetVelPID(&throw_motor,float kp,float ki);
	RobStrideSetLocPID(&throw_motor,float kp);
	vTaskDelay(50);
	RobStrideSetMode(&throw_motor,RobStride_Position);
...
//		{
			RobStridePositionControl(&throw_motor,throw_motor_target_rad);
...
//			throw_vel_pid.pid_out=0.0f;
		...
		if(canSendBuf2[2]>max_2006_cur)		//2006防止过流措施
		{
			cnt++;
			if(cnt>200&&ABS(last_exp_vel-claw_motor.motor.Speed)<10.0f)		//2006过流超过1s时，认为达到稳态，减小电流
				canSendBuf2[2]=max_2006_cur;
		}
		else if(canSendBuf2[2]<-max_2006_cur)
		{
			cnt++;
			if(cnt>200&&ABS(last_exp_vel-claw_motor.motor.Speed)<10.0f)		//电机的期望位置处于相对静止
				canSendBuf2[2]=-max_2006_cur;
		}
		else 
			cnt=0;
